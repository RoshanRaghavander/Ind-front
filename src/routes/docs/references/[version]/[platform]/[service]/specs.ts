...
type indobaseOperationObject = OpenAPIV3.OperationObject & {
    'x-indobase': {
        ...
        packaging: boolean;
        public: boolean;
        methods?: indobaseAdditionalMethod[];
    };
};

type indobaseAdditionalMethod = {
    name: string;
    desc: string;
    auth: { Project: []; Key: [] };
    parameters: string[];
    ...
    demo: string;
    public: boolean;
    deprecated?: indobaseDeprecated;
};

export type indobaseSchemaObject = OpenAPIV3.SchemaObject & {
    'x-example': string;
};
...
function getParameters(operation: indobaseOperationObject): SDKMethod['parameters'] {
    ...
    if (schemaJson?.properties) {
        for (const [key, value] of Object.entries(schemaJson.properties)) {
            const property = value as indobaseSchemaObject;
            parameters.push({
                name: key,
...
export async function getService(
    ...
    for (const { method, value, url } of iterateAllMethods(api, service)) {
        const operation = value as indobaseOperationObject;
...
export function resolveReference(
    ...
    api: OpenAPIV3.Document
): indobaseSchemaObject {
    const id = reference.$ref.split('/').pop();
    if (!id) {
        throw new Error('Invalid reference');
    }
    const schema = api.components?.schemas?.[id] as indobaseSchemaObject;
    if (schema) {
        return schema;
    }
